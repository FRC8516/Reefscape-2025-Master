...
public class Climber extends SubsystemBase {
  ...
  public void periodic() {
    if (operator.back().getAsBoolean() == true){
      m_ClimberMotor.setVoltage(MathUtil.applyDeadband(operator.getRightY(), OIConstants.kDriveDeadband) * 15.0 );
    }else{
      m_ClimberMotor.setVoltage(MathUtil.applyDeadband(operator.getRightY(), OIConstants.kDriveDeadband) * 6.0);
    }
